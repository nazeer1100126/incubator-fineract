...
public class DeviceRegistrationWritePlatformServiceImpl implements DeviceRegistrationWritePlatformService{
    ...
    private void handleDataIntegrityIssues(final String registrationId, final Throwable realCause, @SuppressWarnings("unused") final Exception dve) {

        if (realCause.getMessage().contains("registration_key")) {
            throw new PlatformDataIntegrityException("error.msg.duplicate.device.registration.id", "Registration id : "+registrationId+" already exist.",
                    "name", registrationId);
        }
