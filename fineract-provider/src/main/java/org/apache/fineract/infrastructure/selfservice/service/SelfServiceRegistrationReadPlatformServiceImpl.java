...
public class SelfServiceRegistrationReadPlatformServiceImpl implements SelfServiceRegistrationReadPlatformService {
    ...
    @Override
    public boolean isClientExist(Long clientId, String firstName, String lastName, String mobileNumber, boolean isEmailAuthenticationMode) {
        String sql = "select count(*) from m_client where id = ? and firstname = ? and lastname = ?";
        Object[] params = new Object[] { clientId, firstName, lastName };
        if(!isEmailAuthenticationMode){
            sql = sql + " and mobile_no = ?";
            params = new Object[] { clientId, firstName, lastName, mobileNumber};
        }
